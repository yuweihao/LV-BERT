  ...
  def __init__(self,
    ...
    with tf.variable_scope(scope, default_name="electra"):
      ...
      with tf.variable_scope("encoder"):
        ...
        if custom_layer_list is not None:
          (self.all_layer_outputs, self.attn_maps) = transformer_model_custom_layer(
          ...
          assert False, "need to input custom list"
...
def dynamic_convolution_layer(
                    input_tensor,
                    num_convolution_heads=1,
                    size_per_head=512,
                    value_act=None,
                    initializer_range=0.02,
  ...
  """Performs dynamic convolution

  Args:

    input_tensor: float Tensor of shape [batch_size, seq_length, width].
    num_convolution_heads: int. Number of convolution heads.
    size_per_head: int. Size of each convolution head.
    initializer_range: float. Range of the weight initializer.
    do_return_2d_tensor: bool. If True, the output will be of shape [batch_size
      * input_seq_length, num_convolution_heads * size_per_head]. If False, the
      output will be of shape [batch_size, input_seq_length, num_convolution_heads
      * size_per_head].
    batch_size: (Optional) int. If the input is 2D, this might be the batch size
      of the 3D version of the `input_tensor`.
    input_seq_length: (Optional) If the input is 2D, this might be the seq length
      of the 3D version of the `input_tensor`.

  Returns:
    float Tensor of shape [batch_size, input_seq_length,
      num_convolution_heads * size_per_head]. (If `do_return_2d_tensor` is
      true, this will be of shape [batch_size * from_seq_length,
      num_convolution_heads * size_per_head]).

  Raises:
    ValueError: Any of the arguments or tensor shapes are invalid.
  """

  def reshape_for_conv(input_tensor, batch_size, num_convolution_heads,
                           seq_length, width):
    output_tensor = tf.reshape(
        input_tensor, [batch_size, seq_length, num_convolution_heads*width])
    return output_tensor
  ...
  if len(input_shape) == 3:
    ...
    if batch_size is None or input_seq_length is None:
      raise ValueError(
          "When passing in rank 2 tensors to convolution_layer, the values "
          "for `batch_size`,  `input_seq_length` "
          "must all be specified.")

  input_tensor_2d = reshape_to_matrix(input_tensor)


  GLUL_layer = tf.layers.dense(input_tensor_2d,
        num_convolution_heads * size_per_head,
        activation=None,
        kernel_initializer=create_initializer(initializer_range),
        name="glu_l")
  GLUG_layer = tf.layers.dense(input_tensor_2d,
        num_convolution_heads * size_per_head,
        activation=None,
        kernel_initializer=create_initializer(initializer_range),
        name="glu_g")

  value_layer = tf.multiply(GLUL_layer, tf.math.sigmoid(GLUG_layer))


  # conv input
  key_conv_attn_layer = reshape_for_conv(value_layer, batch_size, num_convolution_heads,
                                input_seq_length, size_per_head)

  separable_conv1d_output_dim = num_convolution_heads * size_per_head

  key_conv_attn_layer = tf.layers.separable_conv1d(key_conv_attn_layer,
      separable_conv1d_output_dim,
      conv_kernel_size,
      padding='same',
      activation=None,
  ...
  conv_kernel_input = key_conv_attn_layer

  conv_kernel_layer = tf.layers.dense(
      conv_kernel_input,
      num_convolution_heads * conv_kernel_size,
      activation=value_act,
      name="conv_attn_kernel")

  conv_kernel_layer = tf.reshape(conv_kernel_layer, 
    [batch_size*input_seq_length*num_convolution_heads, conv_kernel_size, 1])
  ...
  conv_out_layer = tf.reshape(value_layer, [batch_size,input_seq_length,num_convolution_heads * size_per_head])

  conv_out_layer = tf.pad(conv_out_layer, paddings, "CONSTANT")
  # unfold [B,T, N*H, K]
  unfold_conv_out_layer = tf.stack(
    [tf.slice(conv_out_layer, [0, i, 0],[batch_size,input_seq_length,num_convolution_heads * size_per_head]) for i in range(conv_kernel_size)],-1)
  conv_out_layer = tf.reshape(unfold_conv_out_layer,
    [batch_size*input_seq_length*num_convolution_heads ,size_per_head, conv_kernel_size])
  ...
  if do_return_2d_tensor:
    ...
    conv_out_layer = tf.reshape(
        conv_out_layer,
        [batch_size * input_seq_length, num_convolution_heads * size_per_head])
  else:
    # `context_layer` = [B, F, N*H]
    conv_out_layer = tf.reshape(
        conv_out_layer,
  ...
  return conv_out_layer, conv_kernel_layer
...
def attention_layer(from_tensor,
  ...
  qkv_ts = num_attention_heads * size_per_head
  if ghost_head:
    qkv_ts = int(qkv_ts / 2)
  # `query_layer` = [B*F, N*H]
  ...
  if ghost_head:
    q_ghost_weights = tf.get_variable(name='q_ghost_weights', 
      shape=(qkv_ts, ), 
      trainable=True, initializer=create_initializer(initializer_range))
    q_ghost_bias = tf.get_variable(
      name='q_ghost_bias', 
      shape=(qkv_ts, ), 
      trainable=True, initializer=tf.zeros_initializer)
    query_layer = tf.concat([
      tf.einsum('bc, c -> bc', query_layer, q_ghost_weights) + q_ghost_bias, 
      query_layer], -1)
    k_ghost_weights = tf.get_variable(name='k_ghost_weights', 
      shape=(qkv_ts, ), 
      trainable=True, initializer=create_initializer(initializer_range))
    k_ghost_bias = tf.get_variable(
      name='k_ghost_bias', 
      shape=(qkv_ts, ), 
      trainable=True, initializer=tf.zeros_initializer)
    key_layer = tf.concat([
      tf.einsum('bc, c -> bc', key_layer, k_ghost_weights) + k_ghost_bias, 
      key_layer], -1)
    v_ghost_weights = tf.get_variable(name='v_ghost_weights', 
      shape=(qkv_ts, ), 
      trainable=True, initializer=create_initializer(initializer_range))
    v_ghost_bias = tf.get_variable(
      name='v_ghost_bias', 
      shape=(qkv_ts, ), 
      trainable=True, initializer=tf.zeros_initializer)
    value_layer = tf.concat([
      tf.einsum('bc, c -> bc', value_layer, v_ghost_weights) + v_ghost_bias, 
      value_layer], -1)
  # `query_layer` = [B, N, F, H]
...
def transformer_model_custom_layer(input_tensor,
                      attention_mask=None,
                      hidden_size=768,
                      num_hidden_layers=12,
                      num_attention_heads=12,
  ...
  for layer_idx in range(num_hidden_layers):
    with tf.variable_scope("layer_%d" % layer_idx):
      ...
      def dynamic_convolution():
        with tf.variable_scope("dynamic_convolution_layer"):
          dconv_heads = []
          with tf.variable_scope("convolution"):
            dconv_head, probs = dynamic_convolution_layer(
                input_tensor=prev_output,
                num_convolution_heads=num_attention_heads,
            ...
            dconv_heads.append(dconv_head)

          dconv_output = None
          if len(dconv_heads) == 1:
            dconv_output = dconv_heads[0]
          else:
            dconv_output = tf.concat(dconv_heads, axis=-1)

          # Run a linear projection of `hidden_size` then add a residual
          # with `layer_input`.
          with tf.variable_scope("output"):
            dconv_output = tf.layers.dense(
                dconv_output,
                hidden_size,
                kernel_initializer=create_initializer(initializer_range))
            dconv_output = dropout(dconv_output, hidden_dropout_prob)
            dconv_output = layer_norm(dconv_output + prev_output)
        return dconv_output
      ...
      def feed_forward():
        ...
        with tf.variable_scope("output"):
          ff_output = tf.layers.dense(
              ...
              hidden_size,
              kernel_initializer=create_initializer(initializer_range))
          ff_output = dropout(ff_output, hidden_dropout_prob)
          ff_output = layer_norm(ff_output + prev_output)
        return ff_output


      with tf.variable_scope("first_submodule"):
        if custom_layer_list[2*layer_idx] == 0:
          prev_output = dynamic_convolution()
        elif custom_layer_list[2*layer_idx] == 1:
          prev_output = self_attention()
        elif custom_layer_list[2*layer_idx] == 2:
          prev_output = feed_forward()
        else:
          assert False, 'layer number must be in [0, 1, 2]'
        all_layer_outputs.append(prev_output)

      with tf.variable_scope("second_submodule"):
        if custom_layer_list[2*layer_idx+1] == 0:
          prev_output = dynamic_convolution()
        elif custom_layer_list[2*layer_idx+1] == 1:
          prev_output = self_attention()
        elif custom_layer_list[2*layer_idx+1] == 2:
          prev_output = feed_forward()
        else:
          assert False, 'layer number must be in [0, 1, 2]'
        all_layer_outputs.append(prev_output)
