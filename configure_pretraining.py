...
class PretrainingConfig(object):
  ...
  def __init__(self, model_name, data_dir, **kwargs):
    ...
    self.keep_checkpoint_max = 1 # maximum number of recent checkpoint files to keep;
    ...
    if self.model_size in ["small", "medium"]:
      self.embedding_size = 128
      if self.model_size in ["medium"]:
    ...
    if self.max_seq_length == 128:
      self.pretrain_tfrecords = os.path.join(
      data_dir, "pretrain_tfrecords/pretrain_data.tfrecord*")
    elif self.max_seq_length == 512:
      self.pretrain_tfrecords = os.path.join(
      data_dir, "pretrain_tfrecords_512/pretrain_data.tfrecord*")
